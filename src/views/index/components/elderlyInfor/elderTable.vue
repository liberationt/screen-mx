    ...
    <div class="displayFlex">
        <div class="toast">
            <div class="elderInforTitle displayFlex">
                <div class="inforTitleImg" style="cursor: inherit">老人信息列表</div>
...
import {getPersonInfoList,getPersonInfoDetail,getServiceInfo,safeguardMsg,creditCardStatistics,creditCardRecords, getByCondition} from "@/api/elderlyInforDetails"
import {areaCode} from '@/api/areaCode'
import echarts from "echarts"
import Chart from "@/components/Chart"
export default {
    components:{pagination,Chart},
    props:['close', 'begin', 'end'],
    data(){
        return{
            loading:false,
            height:'600px',
    ...
    methods:{
        ...
        getDateList(){
            ...
            if (this.begin && this.end) {
                getByCondition(this.form).then(res=>{
                    if(res.code==0){
                        this.loading = false
    ...
    mounted(){
        if (this.begin && this.end) {
            this.form.ageStart = this.begin
            this.form.ageEnd = this.end
        } else if (this.begin === '100') {
            this.form.ageStart = '100'
            this.form.ageEnd = ''
        }
        this.getDateList()
    }

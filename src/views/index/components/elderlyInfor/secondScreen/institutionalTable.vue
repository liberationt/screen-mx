...
import {getPersonInfoList,getPersonInfoDetail,getServiceInfo,safeguardMsg,creditCardStatistics,creditCardRecords,getOrgTypesList} from "@/api/elderlyInforDetails"
...
export default {
    ...
    props:['close','titleName','orgTypes'],
    data(){
        return{
            loading:false,
            height:'600px',
    ...
    methods:{
        ...
        getDateList(){
            this.loading = true
            getOrgTypesList(this.form).then(res=>{
                if(res.code==0){
                    this.loading = false
    ...
    mounted(){
        this.form.orgType = this.orgTypes
        this.getDateList()
    }
}
</script>
